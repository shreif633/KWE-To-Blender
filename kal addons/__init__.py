...
class KAL_OT_replace_texture(bpy.types.Operator, ImportHelper):
    """Replace a texture in the KCM terrain"""
    bl_idname = "kal.replace_texture"
    bl_label = "Replace Texture"
    bl_options = {'REGISTER', 'UNDO'}

    filename_ext = ".dds;.png;.jpg;.tga"
    filter_glob: StringProperty(default="*.dds;*.png;*.jpg;*.tga", options={'HIDDEN'})

    texture_index: IntProperty(
        name="Texture Index",
        description="Index of texture to replace (0-6)",
        default=0,
        min=0,
        max=6
    )
    ...
    def execute(self, context):
        ...
        mat = obj.data.materials[0]
        tex_node_name = f"Texture_{self.texture_index}"
        ...
        # Load new image
        try:
            new_img = bpy.data.images.load(self.filepath)
            new_img.colorspace_settings.name = 'sRGB'
            tex_node.image = new_img
            self.report({'INFO'}, f"Replaced texture {self.texture_index} with {os.path.basename(self.filepath)}")
...
class KAL_OT_export_texture(bpy.types.Operator, ExportHelper):
    """Export a texture from the KCM terrain"""
    bl_idname = "kal.export_texture"
    ...
    texture_index: IntProperty(
        name="Texture Index",
        description="Index of texture to export (0-6)",
        default=0,
        min=0,
        max=6
    )
    ...
    def execute(self, context):
        ...
        if not tex_node or not tex_node.image:
            self.report({'ERROR'}, f"Texture {self.texture_index} not found or has no image.")
            return {'CANCELLED'}

        # Save the image
        try:
            tex_node.image.filepath_raw = self.filepath
            tex_node.image.file_format = 'PNG'
            tex_node.image.save()
            self.report({'INFO'}, f"Exported texture {self.texture_index} to {self.filepath}")
        except Exception as e:
    ...
    def execute(self, context):
        ...
        try:
            ...
            if not env_texture['ready_for_use'] and env_texture['can_convert']:
                if kcm_file.convert_gtx_to_dds(env_texture['gtx_path'], env_texture['dds_path'], game_path):
...
class KAL_PT_texture_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        for i, tex_idx in enumerate(texture_list):
            ...
            # Replace button
            col = row.column()
            replace_op = col.operator(KAL_OT_replace_texture.bl_idname, text="", icon='FILEBROWSER')
            replace_op.texture_index = i

            # Export button
            col = row.column()
            if has_image:
                export_op = col.operator(KAL_OT_export_texture.bl_idname, text="", icon='EXPORT')
                export_op.texture_index = i
            else:
                col.label(text="", icon='BLANK1')

        # Viewport shading tip
        box = layout.box()
        box.label(text="Viewport Display Tips:", icon='INFO')
        box.label(text="• Use Material Preview mode")
        box.label(text="• Or Rendered mode for best results")
        box.label(text="• Solid mode shows vertex colors only")
...
class KAL_PT_env_texture_browser(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        try:
            ...
            # Texture list (show first 20 for performance)
            box = layout.box()
            box.label(text="Available Textures:", icon='TEXTURE_DATA')
            ...
            if obj and 'kcm_textures' in obj:
                ...
                # Quick replace buttons for terrain textures
                terrain_data = obj.get('kcm_textures', {})
                terrain_texture_list = terrain_data.get('texture_list', [])

                sub_box = box.box()
                sub_box.label(text="Replace Terrain Textures:", icon='FILE_REFRESH')

                for i, tex_idx in enumerate(terrain_texture_list):
                    if tex_idx == 255:
                        continue
                    ...
                    # Show current texture name
                    if tex_idx < len(env_textures):
                        current_name = env_textures[tex_idx]['name']
                        row.label(text=current_name)
                    else:
                        row.label(text="Unknown")

                    # Replace button (will open a popup or use a simple increment)
                    replace_op = row.operator("kal.replace_texture_from_env", text="Replace", icon='FILEBROWSER')
            ...
            for i, texture in enumerate(env_textures[:50]):
                ...
                if obj and 'kcm_textures' in obj:
                    col = row.column()
                    col.scale_x = 0.3
                    use_op = col.operator("kal.replace_texture_from_env", text="Use", icon='PLUS')
...
classes = (
    ...
    KAL_OT_replace_texture,
    KAL_OT_export_texture,
